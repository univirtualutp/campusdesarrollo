...
/**
 * Privacy Subsystem implementation for theme_boosted.
 *
...
namespace theme_boosted\privacy;

use \core_privacy\local\metadata\collection;

/**
 * The boosted theme stores a user preference data.
 *
 * @copyright  2022 koditik.com
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropennavpref)) {
            $preferencestring = get_string('privacy:drawernavclosed', 'theme_boosted');
            if ($draweropennavpref == 'true') {
                $preferencestring = get_string('privacy:drawernavopen', 'theme_boosted');
            }
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_boosted');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_boosted');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_boosted',
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_boosted');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_boosted');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_boosted',
